...
export default async function SalasPage({
  ...
  if (!can(perms, "salas", "view")) redirect("/");

  let query = supabase.from("rooms").select("id, name, description, location");
  ...
  const canCreate = can(perms, "salas", "create");
  const canEdit = can(perms, "salas", "edit");
  const canDelete = can(perms, "salas", "delete");
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-4 p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-serif text-2xl">Salas</h1>
        {canCreate && <RoomFormDialog />}
      </div>
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {(canEdit || canDelete) && (
                <div className="flex gap-1">
                  {canEdit && <RoomFormDialog room={r} />}
                  {canDelete && (
                    <DeleteButton
                      id={r.id}
                      action={deleteRoom}
                      confirmMessage={`Excluir a sala "${r.name}"?`}
                    />
                  )}
                </div>
              )}
