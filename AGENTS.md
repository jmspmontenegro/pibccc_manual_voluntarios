...
- **Testabilidade:** lógica de domínio isolada de I/O (banco, rede) para poder ser testada sem mocks pesados. **Framework de testes: Vitest** (`npm test` = `vitest run`, decidido em 2026-09-08) — leve, roda TS/ESM sem config extra. Funções de domínio ficam em `lib/domain/*.ts` (ex.: `isValidDeclineReason`, `buildSemeandoTempoReason`) com teste colocado ao lado (`*.test.ts`), sem mock de Supabase — isolar a regra de negócio da I/O é o que torna isso testável sem infra pesada.
...
**Confirmação de e-mail: exigida (revertido em 2026-09-08).** "Confirm email" em Authentication → Sign In / Providers → Email fica LIGADO no dashboard do Supabase (passo manual, fora do repo) — decisão explícita do usuário, reverte a nota antiga deste arquivo que assumia o toggle desligado. `signup()` (`app/auth/actions.ts`) trata os dois casos: se `signUp()` não retorna `session` (toggle ligado), mostra "confira seu e-mail" antes de aguardar aprovação; se retorna sessão (toggle desligado, ex. ambiente de teste), pula direto pra "aguardando aprovação". `app/auth/confirm/route.ts` troca o token do e-mail pela sessão e desloga IMEDIATAMENTE em seguida — `status` só é checado dentro de `login()`, não no `app/(app)/layout.tsx`, então um usuário `pending` não pode ficar com sessão válida depois de só confirmar o e-mail. Ver `FLUXO_CADASTRO.md` na raiz do repo pro fluxo completo (cadastro → confirmação → notificação de admin/coordenação → aprovação → notificação do usuário).

**E-mail transacional: Resend, via Vercel Marketplace (2026-09-08).** Supabase Auth só manda e-mail pra quem tá se cadastrando/resetando senha — pra notificar terceiros (admin/coordenação de novo pendente; usuário avisado que foi aprovado) usamos Resend direto. Provisionado com `vercel integration add resend` (não `npm install` avulso — segue o fluxo da skill `vercel:marketplace`); trouxe `RESEND_API_KEY`/`RESEND_EMAIL_DOMAIN` pro `.env.local` via `vercel env pull`. Domínio de envio: `impulsion.com.br` (domínio do usuário, ainda **pendente de verificação DNS** no Resend — até verificar, todo envio real falha com 403 "domain is not verified"; código já está pronto e testado com esse erro exato, só falta o DNS). Skill de referência instalada em `.agents/skills/resend/` (gotchas de idempotency key, formato `{data,error}` sem exceção, etc — ler antes de mexer em `lib/email.ts`).
- `lib/email.ts` — `sendEmail()` (wrapper do SDK, nunca lança, só loga erro) + `sendNewSignupNotification()`/`sendApprovalEmail()` (HTML da marca embutido, sem template do Resend).
- `lib/site-url.ts` — `getSiteUrl()` monta link absoluto pro e-mail (projeto ainda não tem domínio próprio configurado na Vercel — usa `VERCEL_PROJECT_PRODUCTION_URL`, injetada automaticamente; setar `NEXT_PUBLIC_SITE_URL` se/quando tiver domínio).
- Notificação de admin/coordenação dispara em `app/auth/notify-new-signup.ts`, chamada de `app/auth/confirm/route.ts` (fluxo normal, confirm email ligado) e do fallback em `signup()` (confirm email desligado) — **não** usa Supabase Edge Function/DB webhook como o desenho original cogitava; é só uma chamada direta de dentro da própria Server Action/Route Handler, mais simples e sem infra extra.
- E-mail de aprovação dispara direto dentro de `approveUser()` (`app/(app)/admin/usuarios/actions.ts`), depois do `approve_pending_user` RPC ter sucesso.
...
- **Nenhuma operação git além de leitura roda sem pedido explícito do usuário** — nem `git push`, `git merge`, nem `./script_deploy.sh`, mesmo com o código pronto e buildado. Isso vale mesmo quando o usuário pediu pra "terminar"/"finalizar" uma tarefa: terminar a tarefa é deixar o código pronto localmente, não subir pra branch nenhuma. Quem decide se/quando algo vai pro GitHub, leva merge ou vai pra `main` é o usuário, caso a caso — aprovar um deploy uma vez não aprova o próximo. Ao concluir uma tarefa, o padrão é só **reiniciar a aplicação local** (`next start`) pra ele testar, não commitar/empurrar nada.
